...
<ToolContent toolTitle="Signal" caption="Given card ids, provide decks, collections, or check asks for bids for what's missing. Sideroca compatible." />
...
<div class="lg:w-[1024px] lg:max-w-5xl flex flex-col lg:flex-row gap-8 break-normal">
	<form on:submit|preventDefault={() => signal()} class="flex flex-col gap-8">
		...
		<Select name="Behavior" bind:mode={mode} options={["Collection", "Deck", "Asks", "Bids", "Asks and Bids"]} />
        ...
        {/if}
		<Buttons stopButton={true} bind:stopped={stopped} bind:stoppable={stoppable} downloadButton={true} bind:downloadable={downloadable} bind:content={content} name="Queries" />
	</form>
