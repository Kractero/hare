...
<div
	class="mb-4 flex flex-col justify-between gap-8 break-normal lg:w-[1024px] lg:max-w-5xl lg:flex-row"
>
	<div>
...
<p class="my-4 text-xs">
	The thumbs-up column indicates whether they exist or not, the others indicate whether they were a
	legendary in that season or not.
</p>
	...
	<table class="w-full table-auto caption-bottom border-collapse whitespace-normal text-left">
		...
		{#each validHeaders as header}
			<th
				...
				class={`h-12 p-2 font-semibold md:p-4 ${header.includes('S') ? 'underline' : ''}`}
				>{header}</th
			>
		{/each}
		{#each json as nation}
			<tr class="border-b-1 border border-black">
				...
				<td
					class="w-[150px] whitespace-normal break-words break-all p-2 text-xs md:w-auto md:p-4 md:text-base"
				>
					<a
						class="underline"
						rel="noreferrer noopener"
						target="_blank"
						href={`https://${localStorage.getItem('connectionUrl') || 'www'}.nationstates.net/nation=${nation.name}/`}
						>{nation.name}</a
					>
				</td>
					...
					{#if nation[`S${season}`] === true}
						<td class="p-2 md:p-4">
							<a
								rel="noreferrer noopener"
								target="_blank"
								href={`https://${localStorage.getItem('connectionUrl') || 'www'}.nationstates.net/page=deck/card=${nation.id}/season=${season}`}
								><Check color="green" /></a
							>
						</td>
					{:else}
