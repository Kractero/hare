...
<button disabled={!downloadable} type="button" on:click={() => handleDownload(type, (type === "txt" || Array.isArray(content)) ? content : name === "Gold Retriever" ? htmlContent(content, style, sort) : htmlContent(content), name)}
